...
populateDisciplines() {
  ...
  // garante que só tenha 1 listener
  disciplineSelect.onchange = (e) => {
    ...
    if (selectedDiscipline) {
      ...
      if (discipline) {
        ...
        if (Array.isArray(discipline.assuntos)) {
          for (const assunto of discipline.assuntos) {
            options += `<option value="${assunto}">${assunto}</option>`;
          }
        }
        
        // sempre adiciona "Outro"
        options += '<option value="Outro">Outro</option>';
        
        topicSelect.innerHTML = options;
        topicSelect.disabled = false;
      } else {
        topicSelect.innerHTML = '<option value="">Nenhum assunto disponível</option>';
